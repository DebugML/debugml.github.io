...
> CTSketch pushes the frontier of neurosymbolic learning, scaling to tasks involving 1024 inputs, which has never been studied before. 

Neurosymbolic programs refer to the composition of a neural network $M_\theta$ followed by a fixed symbolic program $c$.
The challenge of neurosymbolic learning concerns learning $M_\theta$ with only end-to-end labels of the composite.
Concretely, given an input-output example $(x, y)$, symbolic program $c$, and loss function $L$, we aim to predict the distribution of the outputs $y' = c(M_{\theta}(x))$ while ensuring the loss $L({y'}, y)$ is fully differentiable.
We now discuss prior works on neurosymbolic learning, which provide different techniques for estimating the output distribution of $c$ for the input probabilities predicted by the neural network.

## White- and Black-Box Neurosymbolic Frameworks

Existing neurosymbolic learning techniques can be categorized as either white- or black-box. 
White-box techniques allow access to the internals of the symbolic program. 
<!--to compute the output probability distribution from the input distributions.-->
Differentiable logic programming frameworks are one form of white-box learning technique. 
These frameworks require the symbolic component to be written in logic programming languages, like Prolog (as in [DeepProbLog](https://arxiv.org/abs/1805.10872)) or Datalog (as in [Scallop](https://arxiv.org/abs/2304.04812)), extended with probabilistic facts and differentiable operators. 
These approaches offer heuristics for formulating the output distribution as a differentiable function of input distributions.
<!--
...
On the other hand, black-box frameworks assume no access to the internals of the symbolic component and rely only on the input-output pairs of the program.
...
[A-NeSI](https://arxiv.org/abs/2212.12393) is an algorithm that trains a neural network to approximate the output distribution of the composite.
...
This means the symbolic component can use a large language model (LLM) like GPT-4 to perform reasoning over the network predictions. 
...
We introduce CTSketch, a novel framework that combines the strengths of white- and black-box frameworks. 
...
The sub-programs are evaluated sequentially through the layers of the tree structure, where the first layer of programs corresponds to the leaves and the last sub-program, which predicts the final output, represents the root. 
<!--
