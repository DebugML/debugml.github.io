...
sketching:
  - id: 0
    name: tt
    caption: Tensor-Train (TT) decomposition
  - id: 1
    name: tucker
...
> CTSketch decomposes the symbolic program using sketched tensors that summarize the input-output pairs of each sub-program and perform fast inference via simple tensor operations. 
...
**Sumamry Tensor.**
Each sub-program is summarized using a tensor, where each dimension of the tensor summary corresponds to one of the discrete program inputs. For a sub-program $c_i$ that takes $d$ inputs, the summary $\phi_i$ will be a $d$-dimensional tensor that satisfies $\phi_i[j_1, \dots, j_n] = c(j_1, \dots, j_n)$. 

The sum-4 case will use two different tensors $\phi_1: 10 \times 10$ and $\phi_2: 19 \times 19$, where for both cases $\phi[i, j] = i + j$. 
...
<body style="margin-bottom: 5px">
    ...
    <button id="cpbutton" style="background-color: lightgrey" onclick="showCP()">CP</button> 
      ...
      <figure class="center" style="margin: 0;">
          ...
          <figcaption>CP decomposition</figcaption>
      </figure>
      </div>
    <script>
        function showTT() {
...
Prior to training, CTSketch initializes each summary tensor $\phi_i$ by sampling a subset of the possible inputs or enumerating all input combinations and computing the corresponding outputs. 
After initializing, the algorithm uses the TT decomposition method to obtain sketches.
...
$$v = \sum_a^{10} \sum_b^{10} \sum_x^2 p_1[a] p_2[b] t_1^2[a, x] t_2^1[x, b] $$
...
<div id="overview" style="text-align: center; display: flex; justify-content: space-around; align-items: center;">
  <figure class="center" style="margin: 0;">
          ...
          <figcaption>CTSketch Overview on sum-4</figcaption>
  </figure>
</div>
...
## Evaluation
We evaluate on tasks: 
<ul>
  <li>sum-$n$ ($n \in$ {4, 16, 64, 256, 1024}) and add-$n$ ($n \in$ {1, 2, 4, 15, 100})</li>
...
The per-digit accuracy stays at 17.92% for the next best performaer A-NeSI.
