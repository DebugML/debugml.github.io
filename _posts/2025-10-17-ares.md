...
> However, current error detection methods often fail to detect propagated errors because earlier errors can corrupt downstream judgments.
...
<!--
LLMs often produce reasoning chains with errors that propagate, undermining the final outputs.
...
An <span style='color:orange; font-weight:bold'>**ungrounded error**</span> is when the LLM incorrectly copies information from the context.
A reasoning step is an <span style='color:red; font-weight:bold'>**invalid derivation**</span> if it consists of a mathematically or logically false operation.
Finally, a <span style='color:#880088; font-weight:bold'>**propagated error**</span> is a mistake incurred from previous steps.
...
Current error detection methods, such as LLM judges and Process Reward Models, struggle to catch propagated errors.
This is because they often do not properly decompose the problem, and so are easily distracted by irrelevant or invalid information: there's only so much that one can reasonably expect a single LLM call to achieve!
...
It determines a step's soundness by calculating its entailment probability based _only_ on the set of preceding, sound claims.
...
The main idea in ARES is to pair step-by-step reasoning with step-by-step certification.
At the heart of this is the notion of _logical entailment_: whether a set of premises (i.e., prior claims) entails a given hypothesis (i.e., current claim).
Intuitively, claims that are **not** entailed should be considered erroneous.
By identifying downstream dependencies of these errneous claims, we can then catch propagated errors.


A surprisingly effective way to check entailment is to simply ask an LLM.
However, this is not without pitfalls: including erroneous or invalid premises can easily lead to absurd reasoning!
Instead, we propose to **filter** claims based on their entailment: entailed claims should be autoregressively included in the premise of subsequent steps, whereas non-entailed claims should be dropped.
The figure below illustrates this idea, where each claim is marked as either _sound (✓)_ or _unsound (✗)_ , depending on whether it is entailed.



<!--
A natural way to detect reasoning errors in language is through _entailment_: determining whether a hypothesis logically follows from a given premise. We can ask an LLM to judge this entailment for each step in a reasoning chain.

However, LLMs can make mistakes, and using all previous steps as premises risks carrying earlier errors forward. A more principled approach is to check each claim only against previously verified (sound) information, ensuring that unreliable statements are ignored when evaluating later ones.
-->


<!--
The figure below illustrates this idea in a _binary_ setting: each derived claim is judged as either _sound (✓)_ or _unsound (✗)_ depending on whether it follows logically from the preceding sound claims.
-->
...
<!-- # ANTON: I think we can cut this entire paragraph.
This binary version captures the essence of ARES, which is a way to trace where a reasoning chain remains sound and where it breaks. It’s intuitive, concrete, and easy to visualize, making it ideal for understanding the core mechanism before adding complexity.
-->


While symbolic and mathematical reasoning is often either true or false, natural language reasoning is rarely so black-and-white.
For example, if it's cloudy with high humidity, then it's only *likely* that there will be remain.
In such cases, the premise only *partially* entails the hypothesis.


<!--
However, in natural language, entailment isn’t always black-and-white. For example, if it’s cloudy and the humidity is high, we might say there’s a 60 % chance it will rain, where the hypothesis is *partially* supported.
-->


Such uncertainty is naturally modeled via probabilistic reasoning.

<!-- ANTON: I don't like the remainder of this section. We need to build up how one gets probabilistic scores from an entailment model.

We also need some build-up to talk about weighting each premise combination by its probability.

The rough check points we'd have to hit are:

1. How to get probabilistic scores from the entailment model/oracle. e.g., by repeatedly querying it, or by asking it for a fuzzy response and then doing some kind of Likert score mapping (e.g., "likely" -> 60%)
2. Once you have individual entailment scores, note that the premises themselves are only entailed probabilistically. To account for such uncertainty, the natural idea is to do a weighting where ...
3. This leads to the following definition ...

Sorry for leaving so much vague, but happy to discuss with you (Weiqiu) on Slack for drafts of this section's tail.

-->

To handle such uncertainty, we extend ARES to a probabilistic formulation. Instead of marking each step as simply sound or unsound, we estimate its expected entailment probability, conditioned on preceding sound claims. This defines the probabilistic soundness of each reasoning step, inspired by how humans naturally disregard dubious premises and reason under uncertainty.

**Definition [Soundness]** A claim's *soundness* is how much it is entailed *on average* across all sound combinations of preceding claims. Its **reasoning stability rate** $\tau_k$ quantifies this probability for the $k$-th claim.  
{: .notice--info}

This probabilistic formulation brings two key benefits:  
1. **Fuzzy entailment:** it models partial support rather than binary truth.  
2. **Error tolerance:** by averaging entailment rates over subsets of sound claims, it mitigates the impact of earlier mistakes.



<!-- ## Detecting Errors via Autoregressive Reasoning Entailment Stability (ARES)

An intuitive tool to use to detect natural language errors is entailment, which determines whether a hypothesis holds given a premise.
We can use an LLM to judge the entailment.
A natural thought for judging a reasoning chain would be checking the entailment of a claim given all preceding claims as the premise.

However, as LLMs can make errors, judging entailment for later claims with such unsound information in the premise is unreliable.
It would then be more principled to use only previous sound information as the premise.

In addition, entailment for natural language is often fuzzy.
For example, given that it’s cloudy and the humidity is high, we might say there’s about a 60% chance it will rain.
In natural language, entailment isn't always black and white; it often reflects degrees of likelihood rather than absolute truth.

One natural way to both be error resistent and capture fuzziness is to use a probabilistic entailment model and certify the soundness of claims inductively.

The figure below illustrates how ARES verifies reasoning chains by checking each derived claim against previously verified ones. In this **binary case** for conceptualization, a step is either judged sound (✓) or unsound (✗) depending on whether it follows logically from the preceding, sound claims.
...
While the above definition of soundness is convenient to define, it is intractable to compute!
...
<!-- ANTON: The above isn't a very good description, but hopefully it conveys the spirit of what we should have -->
...
<div class="notice--success">
  ...
  <strong>Algorithm. Estimate Autoregressive Reasoning Entailment Stability (ARES) $\tau_k$ for each derived claim in a reasoning chain.</strong> <br><br>

  Suppose the reasoning chain consists of base claims $(B_1, \ldots, B_n)$ and derived claims $(D_1, \ldots, D_m)$.<br><br>

  <div style="margin-left: 16px;">
    <strong>Step 1.</strong> <em>Sample base claims as premises.</em><br>
    Draw $N$ random subsets of the $n$ base claims, where each $B_i$ is included independently with its prior probability&nbsp;$p_i$.<br><br>

    <strong>Step 2.</strong> <em>Iterate through all derived claims $D_k$, from $k = 1$ to $m$:</em><br>
    <div style="margin-left: 20px;">
      <strong>(a)</strong> For each sample, compute the entailment probability&nbsp;$p_k$ that the current derived claim $D_k$ is supported by the included previous claims (bases and prior derived claims).<br>
      <strong>(b)</strong> Average these entailment probabilities across all $N$ samples to estimate its reasoning stability rate&nbsp;($\tau_k$).<br>
      <strong>(c)</strong> For future steps, include this new derived claim in each sample with probability&nbsp;$p_k$ (Bernoulli sampling).<br>
...
<br>
  <div style="margin-left: 16px;">
    ...
    then with probability at least&nbsp;$(1 - \delta)$, the estimated stability rate&nbsp;$\hat{\tau}_r$  
    for any claim&nbsp;$r$ satisfies&nbsp;$|\hat{\tau}_r - \tau_r| \le \varepsilon$.
  </div>
</div>
...
In parallel, we sample from the entailment rate for the claim in each sample to decide whether or not to include it when certifying future claims for that sample.
